...
FORWARD_MAP=(
  "${WOTLK_AUTH_PUBLIC_PORT:-3724}:${WOTLK_AUTH_BACKEND_PORT:-3724}"
  "${WOTLK_WORLD_PUBLIC_PORT:-8443}:${WOTLK_WORLD_BACKEND_PORT:-8085}"
)
...
cleanup_legacy_world_443() {
  ...
  iptables -t nat -D PREROUTING -i "$PUB_IFACE" -p tcp --dport 443 -j DNAT --to-destination "${HOME_IP}:8085" 2>/dev/null || true
...
for mapping in "${FORWARD_MAP[@]}"; do
  ...
  add_rule nat PREROUTING -i "$PUB_IFACE" -p tcp --dport "$public_port" -j DNAT --to-destination "${HOME_IP}:${backend_port}"
  add_rule filter FORWARD -i "$PUB_IFACE" -o "$WG_IFACE" -p tcp -d "$HOME_IP" --dport "$backend_port" -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
  add_rule filter FORWARD -i "$WG_IFACE" -o "$PUB_IFACE" -p tcp -s "$HOME_IP" --sport "$backend_port" -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
  add_rule nat POSTROUTING -o "$WG_IFACE" -p tcp -d "$HOME_IP" --dport "$backend_port" -j MASQUERADE
done
