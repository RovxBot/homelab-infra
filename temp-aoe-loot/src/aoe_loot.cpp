...
bool AOELootServer::CanPacketReceive(WorldSession* session, WorldPacket& packet)
{
    ...
    for (Creature* creature : nearbyCorpses)
    {
        ...
        for (size_t i = 0; i < loot->items.size(); ++i)
        {
            ...
            // Check item quality - only merge items at or below max quality
            const LootItem& lootItem = loot->items[i];
            if (lootItem.itemid)
            {
                ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(lootItem.itemid);
                if (itemTemplate)
                {
                    uint32 maxQuality = sConfigMgr->GetOption<uint32>("AOELoot.MaxQuality", 1);
                    if (itemTemplate->Quality <= maxQuality)
                    {
                        itemsToAdd.push_back(lootItem);
                    }
                }
            }
        }
