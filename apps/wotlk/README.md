...
## Cloudflare DNS (WoW Realm)

Cloudflare can be used for DNS, but cannot proxy WoW traffic (non-HTTP). For WoW ports, Cloudflare must be **DNS-only**.
...
It also upserts the realm row to `wow.risk-lenok.ts.net:8085` and marks it online.
